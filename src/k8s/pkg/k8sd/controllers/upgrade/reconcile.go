...
func (c *Controller) reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	switch {
	case upgrade.Status.Phase == crdsv1.UpgradePhaseNodeUpgrade:
		return c.reconcileNodeUpgrade(ctx, &upgrade)
	case upgrade.Status.Phase == crdsv1.UpgradePhaseFeatureUpgrade:
...
func (c *Controller) reconcileNodeUpgrade(ctx context.Context, upgrade *crdsv1.Upgrade) (ctrl.Result, error) {
	...
	if err := c.transitionTo(ctx, upgrade, crdsv1.UpgradePhaseFeatureUpgrade); err != nil {
		return ctrl.Result{}, fmt.Errorf("failed to transition to %q phase: %w", crdsv1.UpgradePhaseFeatureUpgrade, err)
	}

	log.Info(fmt.Sprintf("Transitioned to %q phase.", crdsv1.UpgradePhaseFeatureUpgrade))
	return ctrl.Result{}, nil
}

// reconcileFeatureUpgrade triggers feature controllers to reconcile
// and waits for them to finish.
func (c *Controller) reconcileFeatureUpgrade(ctx context.Context, upgrade *crdsv1.Upgrade) (ctrl.Result, error) {
	...
	if err := c.transitionTo(ctx, upgrade, crdsv1.UpgradePhaseCompleted); err != nil {
		return ctrl.Result{}, fmt.Errorf("failed to transition to %q phase: %w", crdsv1.UpgradePhaseCompleted, err)
	}

	log.Info(fmt.Sprintf("Transitioned to %q phase.", crdsv1.UpgradePhaseCompleted))
	return ctrl.Result{}, nil
}
...
func (c *Controller) transitionTo(ctx context.Context, upgrade *crdsv1.Upgrade, phase crdsv1.UpgradePhase) error {
